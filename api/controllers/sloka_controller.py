...
def get_slokas_by_kanda_sarga(kanda_number, sarga_number, sloka_number):
    try:
        ...
        logger.debug("Processing for kanda %s, sarga %s", kanda_number,
                     sarga_number)
        if not kanda:
            logger.error("Unable to get %s kanda details from the corpus ",
                         kanda_number)
        ...
        if not sarga:
            return jsonify({
                "error":
                f"Sarga '{sarga_number}' not found for Kanda '{kanda_number}'"
            }), 404
        ...
        meaning = sarga.slokas[
            sloka_number].meaning  # Replace with your logic to get the meaning
        translation = sarga.slokas[
            sloka_number].translation  # Replace with your logic to get the translation

        kanda_name = ramayanam_data.kandaDetails.get(kanda_number,
                                                     {}).get('name')
        if not kanda_name:
            return jsonify({'error': 'Invalid Kanda number'}), 404
        ...
        sloka = Sloka(sloka_id=f'{sloka.id}',
